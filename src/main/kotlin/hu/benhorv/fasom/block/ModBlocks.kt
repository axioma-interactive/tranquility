package hu.benhorv.fasom.block
...
object ModBlocks {
    ...
    private fun registerBlock(name: String, blockSettings: AbstractBlock.Settings) : Block {
        val key = RegistryKey.of(RegistryKeys.BLOCK, Identifier.of(Fasom.MOD_ID, name))
    ...
    private fun registerBlockItem(name: String, block: Block) {
        val key = RegistryKey.of(RegistryKeys.ITEM, Identifier.of(Fasom.MOD_ID, name))
        val item = BlockItem(block, Item.Settings().registryKey(key))
        Registry.register(Registries.ITEM, key, item)
    }

    fun registerModBlocks() {
        logger.info("Registering Mod Blocks for " + Fasom.MOD_ID)
